...
let listing1 = {
  ...listings[1], isSold: false, currentOwner:"Jane Doe"
};
...
let priceString = listings.price;

let priceNumber = parseFloat(priceString);

let fee;
 if (priceNumber <= 450000) {
    fee = priceNumber * 0.025;
  } else {
    fee = priceNumber * 0.02;
  }

  // Round the fee and return it
 ...
 */
//WRITE YOUR CODE BELOW
let listingAscendingly = listings.slice();

listingAscendingly.sort(function (a, b){
  if (a.price < b.price) {
    return -1; 
...
let newBuiltListings = [];

for (let i = 0; i < listings.length; i++) {
  let listing = listings[i];
  let builtIn = listing.propertySummary.builtIn;

  if (builtIn.length === 4) {
    let year = parseInt(builtIn);
    if (year >= 1990) {
      newBuiltListings.push(listing);
    }
  }
};
